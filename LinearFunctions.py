...
class Definition(Scene):
	def construct(self):
		...
		defination = Text("A linear function is defined as a function\nthat has either one or two variables without exponents.",
		                  font_size=40,
		                  should_center=True)
		exampleCaption = Text("Here is an example of a linear function", font_size=40, should_center=False).move_to(UP * 2)
		...
		self.play(AddTextLetterByLetter(defination, run_time=1.5))
		self.wait(2.5)
		self.play(Unwrite(defination))
...
class Plotting(Scene):
	def construct(self):
		...
		exampleCaption = Text("Let's take our previous example:- ", font_size=28).next_to(title, DOWN, buff=0.3).shift(LEFT * 2.5)
		...
		independ = Text("Here we can take any number for x\n as it is an independent variable", font_size=30, should_center=True).next_to(table, RIGHT * 1.5)
		depend = Text("However, for y values we need to solve\n the function by evaluating \nit with the x values", font_size=25, should_center=True).next_to(table, RIGHT * 1.5)

		evaluationCaption = Text("After putting each value in our function\n and solving it we get the \nfollowing y values", font_size=27, should_center=True).next_to(table, RIGHT * 1.5)

		coordCaption = Text("According to the table, we now have 3 points:- ", font_size=20, should_center=True).next_to(table, DOWN, buff=-0.5)
		...
		plotCaption = Text("Now we can plot these points on a graph", font_size=20, should_center=True).next_to(table, DOWN, buff=-0.5)
